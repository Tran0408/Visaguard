...
async def inbound_email(
    ...
    if not user and from_address:
        user_q = await db.execute(select(User).where(User.email == from_address))
        user = user_q.scalar_one_or_none()
    ...
    # Kick off LLM parsing + shift insert in background so Postmark gets fast 200.
    asyncio.create_task(_process_email_async(payload, user.id, log.id))
